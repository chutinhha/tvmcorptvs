...
<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="UpdateWorkflowItemWithESignMetaDataEditor.ascx.cs" Inherits="TVMCORP.TVS.WORKFLOWS.Controls.UpdateWorkflowItemWithESignMetaDataEditor" %>
...
<TABLE border="0" width="95%" cellspacing="0" cellpadding="0" class="ms-formtable" >
	<wssuc:InputFormSection runat="server" Title="Update workflow item with ESignMetadata">
		<Template_Description>
			<SharePoint:EncodedLiteral ID="EncodedLiteral1" runat="server" text="This action will update Workflow Item with returning ESign's metadata." EncodeMethod='HtmlEncode'/>
		</Template_Description>
			    ...
			    <Template_Control>
			        <asp:DropDownList ID="ddlMetadataProperty" runat="server" CssClass="ms-long" /> 
			         <wssawc:InputFormRequiredFieldValidator ID="ddlMetadataPropertyRequiredFieldValidator" 
					                                        ...
					                                        Display="Dynamic" 
					                                        ErrorMessage="Please select a metadata" 
					                                        Runat="server"
                                                            ValidationGroup="SubmitValidate"/>
			    </Template_Control>
			</wssuc:InputFormControl>
