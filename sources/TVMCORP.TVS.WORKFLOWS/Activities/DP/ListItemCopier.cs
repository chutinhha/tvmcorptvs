...
{
    /// <summary>
    /// represents an error that occured during copy operation
    /// </summary>
    public class DPWorkflowCopyItemException : ApplicationException
    {
        public DPWorkflowCopyItemException(string message, Exception inner)
            : base(message, inner)
        {
    ...
    /// </summary>
    public class ListItemCopier : IDisposable
    {
        ...
        public virtual int Copy()
        {
            ...
            if (_finished)
                throw new DPWorkflowCopyItemException("Copy() was already called on this object. Only one call to Copy() is allowed per object instance!", null);
        ...
        protected virtual int CopyFile()
        {
            ...
            CopyFieldValues(this.SourceItem, myFile.Item);
        ...
        protected virtual int CopyListItem()
        {
            ...
            if (SourceList.EnableAttachments && DestinationList.EnableAttachments)
            {
                ...
                {
                    AttachmentInfo[] attachments = Common.GetListItemAttachments(SourceItem);

                    foreach (AttachmentInfo ai in attachments)
                    {

                        destItem.Attachments.Add(ai.FileName, Common.ReadFully(ai.Stream, Convert.ToInt32(ai.Stream.Length)));
        ...
        static void CopyFileWithHistoryCrossSite(SPFile sourceFile, SPFolder destination, string destinationUrl)
        {
            ...
            catch (Exception e) //if error copying
            {
