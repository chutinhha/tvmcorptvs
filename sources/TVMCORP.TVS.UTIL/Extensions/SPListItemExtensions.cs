    ...
    public static class SPListItemExtensions
    {
        ...
        private static void CopyMetadata(SPListItem listItem, SPListItem destinationItem, string[] ignoreFields, SPFieldCollection destFields)
        {
            ...
            while (retry < 5 & !success)
            {
                        ...
                        if (sourceFields.ContainsField(destField.Title))
                        {
                            ...
                            if (!sourceField.ReadOnlyField && !sourceField.Hidden && !ignoreFields.Contains(destField.Title))
                            {
                                ...
                                catch
                                {
                                    ...
                                    Utility.LogInfo("CopyMetaData " + destField.Title + " is not metadata of " + listItem.Title + ":" + listItem.Name, "TVMCORP.TVS.UTIL");
                                }
                            }
                        }
                    }
                ...
                {
                    Utility.LogError(ex.Message + ex.StackTrace, "TVMCORP.TVS.UTIL");
                    Thread.Sleep(1000);
                }
            }
        }
        ...
        public static T GetCustomSettings<T>(this SPListItem listItem, TVMCORPFeatures featureName)
        {
            return listItem.GetCustomSettings<T>(featureName, true);
        }

        public static T GetCustomSettings<T>(this SPListItem listItem, TVMCORPFeatures featureName, bool lookupInParent)
        {
            string strKey = Utility.BuildKey<T>(featureName);
            string settingsXml = listItem.GetCustomProperty(strKey);

            //Utility.LogError(settingsXml, CCIappFeatureNames.CCIappEEC);
        ...
        public static void SetCustomSettings<T>(this SPListItem listItem, TVMCORPFeatures featureName, T settingsObject)
        {
            string strKey = Utility.BuildKey<T>(featureName);
        ...
        public static void RemoveCustomSettings<T>(this SPListItem listItem, TVMCORPFeatures featureName)
        {
            string strKey = Utility.BuildKey<T>(featureName);
        ...
        //        {
        //            string text = Utility.ExtractWordContent(listItem.File);
