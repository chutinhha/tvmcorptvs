...
namespace WordDocumentGenerator.Client
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            GenerateMyDocumentUsingDocWithTableGenerator();
            return;
            ...
            GenerateDocumentUsingDocWithTableAndChartsGenerator();
        ...
        private static void GenerateMyDocumentUsingDocWithTableGenerator()
        {
            // Test refreshable document with table generation from template("Test_Template - 2.docx")
            DocumentGenerationInfo generationInfo = GetDocumentGenerationInfo("SampleDocumentWithTableGenerator", "1.0", GetDataContext1(),
                                                    "My_sample.docx", false);
            MySampleDocumentWithTableGenerator sampleDocumentWithTableGenerator = new MySampleDocumentWithTableGenerator(generationInfo);
            byte[] result = sampleDocumentWithTableGenerator.GenerateDocument();
            WriteOutputToFile("My_sample_Out.docx", "My_sample.docx", result);
        }
        ...
        private static MyAddress GetDataContext1()
        {
            MyAddress order = new MyAddress();

            order.Name = "Test Order";
            order.Date = DateTime.Now;
            order.Phones = new System.Collections.Generic.List<Phone>();

            order.Phones.Add(new Phone() { Label = "Home", Number = "0904199253" });
            order.Phones.Add(new Phone() { Label = "Work", Number= "08-39328000" });
            order.Phones.Add(new Phone() { Label = "emergency", Number = "08-113" });
            order.Phones.Add(new Phone() { Label = "xxx", Number = "08-115" });
